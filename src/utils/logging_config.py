...
class ColoredFormatter(logging.Formatter):
    ...
    EMOJI = {"DEBUG": "🔍", "INFO": "✅", "WARNING": "⚠️", "ERROR": "❌", "CRITICAL": "🚨", "SECURITY": "🔐"}

    def format(self, record: logging.LogRecord) -> str:
        # Get emoji for log level
        is_security = getattr(record, "security", False)
        emoji = self.EMOJI.get("SECURITY", "") if is_security else self.EMOJI.get(record.levelname, "")
...
def setup_logging(
    ...
    if enable_file:
        ...
        security_formatter = logging.Formatter(
            fmt="%(asctime)s - %(levelname)s - [SECURITY] %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            if duration > self.threshold_ms:
                ...
                self.logger.debug(f"⚡ Performance: {self.operation} completed in {duration:.2f}ms")
...
def init_app_logging(app):
    ...
    _root_logger = setup_logging(app_name="niknotes", log_level=log_level, enable_file=enable_file)
