...
from src.utils.logging_config import get_logger, init_app_logging
...
def _apply_rate_limits(app, config):
    ...
    # Apply all limits
    all_limits = {**critical_limits, **sensitive_limits, **moderate_limits, **standard_limits}
    ...
    for endpoint, limit in all_limits.items():
        try:
            ...
            category = (
                "CRITICAL"
                if endpoint in critical_limits
                else "SENSITIVE"
                if endpoint in sensitive_limits
                else "MODERATE"
                if endpoint in moderate_limits
                else "STANDARD"
            )
...
def _init_blueprints(service_container):
    ...
    from src.blueprints import api, auth, main, trips
