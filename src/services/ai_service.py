...
class AIService:
    ...
    def generate_packing_suggestions(self, trip: Trip) -> list[str]:
        ...
        if not weather_summary and self.weather.enabled:
            weather_summary = self.weather.get_weather_summary(trip.destination, trip.start_date, trip.end_date)
            if weather_summary:
        ...
        if cached_suggestions:
            logger.info(
                f"Cache HIT for trip {trip.id}: {trip.destination} ({trip.duration}d, {len(trip.travelers)} travelers)"
            )
