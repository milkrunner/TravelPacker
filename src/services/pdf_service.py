...
class PDFService:
    ...
    def generate_packing_list_pdf(trip: Trip, items: list[PackingItem], progress: dict) -> BytesIO:
        ...
        normal_style = ParagraphStyle(
            "CustomNormal", parent=styles["Normal"], fontSize=10, textColor=colors.HexColor("#4b5563")
        )
        ...
        progress_text = Paragraph(f"<b>{completion:.0f}% Complete</b> - {packed} of {total} items packed", normal_style)
        ...
        for category, category_items in sorted(items_by_category.items()):
            ...
            items_table = Table(table_data, colWidths=[0.4 * inch, 3 * inch, 0.6 * inch, 2 * inch])
        ...
        footer_style = ParagraphStyle(
            "Footer", parent=styles["Normal"], fontSize=8, textColor=colors.HexColor("#9ca3af"), alignment=TA_CENTER
        )
        footer_text = Paragraph(
            f"Generated by NikNotes on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", footer_style
        )
