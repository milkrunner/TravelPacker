...
class ContentSanitizer:
    ...
    RICH_ATTRIBUTES = {"a": ["href", "title"], "span": ["class"]}
    ...
    @staticmethod
    def sanitize_strict(text: str | None) -> str:
        """
    ...
    @staticmethod
    def sanitize_basic(text: str | None) -> str:
        """
    ...
    @staticmethod
    def sanitize_standard(text: str | None) -> str:
        """
    ...
    @staticmethod
    def sanitize_rich(text: str | None) -> str:
        """
    ...
    def sanitize_url(url: str | None) -> str:
        ...
        try:
            ...
            if not any(url.lower().startswith(f"{protocol}:") for protocol in ContentSanitizer.ALLOWED_PROTOCOLS):
                return ""

            # Clean the URL
            cleaned = bleach.clean(
                url, tags=[], attributes={}, protocols=ContentSanitizer.ALLOWED_PROTOCOLS, strip=True
            )
    ...
    def sanitize_trip_data(data: dict) -> dict:
        ...
        if "travelers" in data:
            sanitized["travelers"] = [
                ContentSanitizer.sanitize_strict(traveler) for traveler in data.get("travelers", []) if traveler
            ]

        if "activities" in data:
            sanitized["activities"] = [
                ContentSanitizer.sanitize_standard(activity) for activity in data.get("activities", []) if activity
            ]
    ...
    def sanitize_item_data(data: dict) -> dict:
        ...
        passthrough_fields = ["category", "quantity", "is_packed", "is_essential", "ai_suggested", "display_order"]
        for field in passthrough_fields:
            if field in data:
                sanitized[field] = data[field]
