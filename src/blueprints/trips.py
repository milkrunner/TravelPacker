...
from src.models.trip import TransportMethod, TravelStyle
...
def new_trip():
    ...
    if request.method == "POST":
        ...
        sanitized_travelers = [ContentSanitizer.sanitize_strict(traveler) for traveler in raw_travelers if traveler]

        # Sanitize activities
        raw_activities = data.get("activities", "").split(",") if data.get("activities") else []
        sanitized_activities = [
            ContentSanitizer.sanitize_standard(activity.strip()) for activity in raw_activities if activity.strip()
        ]
        ...
        if trip.id:
            ...
            if weather_service.enabled:
                weather_summary = weather_service.get_weather_summary(trip.destination, trip.start_date, trip.end_date)
                if weather_summary:
        ...
        if use_ai and trip.id:
            ...
            for suggestion in suggestions:
                ...
                packing_service.create_item(
                    trip_id=trip.id, name=item_name, category=category, quantity=quantity, ai_suggested=True
                )
    ...
    return render_template("new_trip.html", travel_styles=travel_styles, transport_methods=transport_methods)
...
def save_as_template(trip_id):
    ...
    try:
        ...
        return f"Error saving template: {e!s}", 500
...
def delete_trip(trip_id):
    ...
    try:
        ...
        return f"Error deleting trip: {e!s}", 500
