...
from src.database import close_session, get_session, init_db
...
def test_google_user_creation_with_legacy_not_null(google_service):
    ...
    try:
        ...
        assert db_user.password_hash is None, "OAuth users should have NULL password_hash"
...
def test_google_user_link_existing_email(google_service):
    ...
    try:
        ...
        existing_user = DBUser(id=user_id, username="existing_user", email="existing@example.com")
    ...
    try:
        ...
        assert db_user.oauth_provider == "google"
        assert db_user.oauth_id == user_info["oauth_id"]
        assert db_user.profile_picture == user_info["profile_picture"]
        # password_hash should still exist (not overwritten)
        assert db_user.password_hash is not None, "Original password_hash should be preserved"
    finally:
        close_session()
