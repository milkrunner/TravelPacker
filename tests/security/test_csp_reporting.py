...
class TestCSPReportingEndpoint:
    ...
    def test_csp_report_accepts_valid_report(self, client):
        ...
        response = client.post("/csp-report", data=json.dumps(report), content_type="application/json")
    ...
    def test_csp_report_handles_minimal_report(self, client):
        ...
        report = {"csp-report": {"violated-directive": "script-src 'self'", "blocked-uri": "https://evil.com/test.js"}}
    ...
    def test_csp_report_handles_invalid_json(self, client):
        ...
        response = client.post("/csp-report", data="{ invalid json", content_type="application/json")
        assert response.status_code == 204

    def test_csp_report_handles_missing_csp_report_key(self, client):
        """Test that endpoint handles reports without csp-report key"""
        report = {"violated-directive": "script-src 'self'", "blocked-uri": "https://evil.com/test.js"}
...
class TestCSPIntegration:
    ...
    def test_multiple_violations_same_session(self, client):
        ...
        reports = [
            {"csp-report": {"violated-directive": "script-src 'self'", "blocked-uri": f"https://evil.com/script{i}.js"}}
            for i in range(10)
        ]
...
class TestCSPPerformance:
    ...
    def test_large_report_payload(self, client):
        ...
        report = {
            "csp-report": {"violated-directive": "script-src 'self'", "blocked-uri": f"https://evil.com/{large_data}"}
        }
