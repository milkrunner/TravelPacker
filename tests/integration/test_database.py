...
from src.database.repository import PackingItemRepository, TripRepository
...
def test_trip_repository_get(setup_database, test_user):
    ...
    trip = Trip(destination="Paris, France", start_date="2025-11-01", end_date="2025-11-05", travelers=["Adult"])
...
def test_packing_item_repository_create(setup_database, test_user):
    ...
    item = PackingItem(trip_id=saved_trip.id, name="Passport", category=ItemCategory.DOCUMENTS, is_essential=True)
